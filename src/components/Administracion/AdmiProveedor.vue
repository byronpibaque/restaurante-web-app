<template>
  <div class="responsive-dialog">
    <span class="text-title">Administración de proveedores</span>
    <div class="grid">
    ...
    <DataTable :value="datos" class="responsive-datatable">
      ...
      <Column field="razon_social_proveedor" header="Razon social"></Column>
...
export default {
  ...
  methods: {
    ...
    openPersonaDialog() {
      this.$refs.AdmiPersonaDialog.openDialogProveedor();
    },
    async listar(filtro = "") {
      try {
        let params = {}
   ...
   async ver(item) {
      ...
      this.nuevoRegistro.id_proveedor = item.id_proveedor;
      this.nuevoRegistro.id_persona = item.persona_id;
      this.nuevoRegistro.razon_social_proveedor = item.razon_social_proveedor;
      this.$refs.AdmiPersonaDialog.openDialogViewProveedor();
    },
    async editar(item) {
      ...
      this.nuevoRegistro.id_persona = item.persona_id;
      this.nuevoRegistro.razon_social_proveedor = item.razon_social_proveedor;
      this.$refs.AdmiPersonaDialog.openDialogUpdateProveedor();
    },
    async eliminar(item) {
      try {
        const response = await this.$api.delete(`${this.endpoint}/${item.id_proveedor}`);
        const resp = response.data;
        if (resp.success === true) {
          this.$swal.fire({
            icon:"success",
    ...
    async restaurar(item){
      try {
        item.estado="Activo";
        const response = await this.$api.put(`${this.endpoint}/${item.id_proveedor}`,item);
        const resp = response.data;
        if (resp.success === true) {
          this.$swal.fire({
            icon:"success",
