<template>
  <div class="responsive-dialog">
    <span class="text-title">Administración de roles</span>
  ...
  methods: {
    async agregar() {
      try {
        ...
        const resCab = await this.$api.post("/admiRol", this.nuevoRegistro);
    ...
    async listar() {
      try {
        const response = await this.$api.post("/admiRol/filter", { estado: "Activo" });
    ...
    editar(item) {
      ...
      this.nuevoRegistro.nombre = item.nombre;
      this.nuevoRegistro.id_rol = item.id_rol;
      this.mostrarDialogo = true;
      this.update = true;
    },
    async actualizar() {
      try {
        ...
        const resCab = await this.$api.put(
          "/admiRol/" + this.nuevoRegistro.id_rol,
          this.nuevoRegistro
        );
        const respuesta = resCab.data;
        if (respuesta.success === true) {
    ...
    async eliminar(item) {
      try {
        const response = await this.$api.delete("/admiRol/" + item.id_rol);
        const resp = response.data;
        if (resp.success === true) {
          this.listar();
        }
