<template>
  <div class="responsive-dialog">
    <span class="text-title">Administración de categorías</span>
  ...
  methods: {
    async agregar() {
      try {
        ...
        const resCab = await this.$api.post(
          "/admiCategoria",
          this.nuevoRegistro
        );
    ...
    async listar() {
      try {
        const response = await this.$api.post("/admiCategoria/filter", {
          estado: "Activo",
        });
    ...
    editar(item) {
      ...
      this.nuevoRegistro.nombre = item.nombre;
      this.nuevoRegistro.descripcion = item.descripcion;
      this.nuevoRegistro.id_categoria = item.id_categoria;
      this.mostrarDialogo = true;
      this.update = true;
    },
    async actualizar() {
      try {
        ...
        const resCab = await this.$api.put(
          "/admiCategoria/" + this.nuevoRegistro.id_categoria,
          this.nuevoRegistro
        );
        const respuesta = resCab.data;
        if (respuesta.success === true) {
    ...
    async eliminar(item) {
      try {
        const response = await this.$api.delete(
          "/admiCategoria/" + item.id_categoria
        );
        const resp = response.data;
        if (resp.success === true) {
          this.listar();
