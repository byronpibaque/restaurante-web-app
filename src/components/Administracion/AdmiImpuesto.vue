<template>
  <div class="responsive-dialog">
    <span class="text-title">Administración de los impuestos</span>
    ...
    >
      <div class="dialog-content">
        ...
        <InputText
          :disabled="view"
          id="porcentaje"
          v-model="nuevoRegistro.porcentaje"
          placeholder="Nombre"
          class="input-spacing"
        />
        <InputText
  ...
  methods: {
    async agregar() {
      try {
        ...
        const resCab = await this.$api.post("/admiImpuesto", this.nuevoRegistro);
    ...
    async listar() {
      try {
        const response = await this.$api.post("/admiImpuesto/filter", {
          estado: "Activo",
        });
    ...
    ver(item) {
      ...
      this.nuevoRegistro.porcentaje = item.porcentaje;
    ...
    editar(item) {
      ...
      this.nuevoRegistro.porcentaje = item.porcentaje;
      this.nuevoRegistro.descripcion = item.descripcion;
      this.nuevoRegistro.id_impuesto = item.id_impuesto;
      this.mostrarDialogo = true;
      this.update = true;
    },
    async actualizar() {
      try {
        ...
        const resCab = await this.$api.put(
          "/admiImpuesto/" + this.nuevoRegistro.id_impuesto,
          this.nuevoRegistro
        );
        const respuesta = resCab.data;
        if (respuesta.success === true) {
    ...
    async eliminar(item) {
      try {
        const response = await this.$api.delete("/admiImpuesto/" + item.id_impuesto);
        const resp = response.data;
        if (resp.success === true) {
          this.listar();
        }
