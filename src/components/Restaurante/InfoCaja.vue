<template>
  <div class="responsive-dialog">
    <span class="text-title">Información de Cajas</span>
    <div class="grid">
      ...
      <div class="col-12 md:col-6 lg:col-3 xl:col-3">
        <AutoComplete id="sucursal" placeholder="Filtra por sucursal:" v-model="sucursal" :suggestions="sucursalArray"
          @complete="searchSucursal" dropdown @keypress.enter="listar(estado_sistema, sucursal)" field="label" />
      </div>
    ...
    <Dialog :header="dialogHeader" v-model:visible="mostrarDialogo" @hide="limpiar" class="responsive-dialog1">
      ...
      <div class="dialog-content">
        <InputText :disabled="view" id="caja" v-model="nuevoRegistro.nombre" placeholder="Caja: CJ#1"
          class="input-spacing" />
      </div>
      <div class="dialog-content">
        <InputText :disabled="view" id="caja" v-model="nuevoRegistro.pto_emision" placeholder="Pto Emision: 001"
          class="input-spacing" />
      </div>
    ...
    <DataTable :value="datos" class="responsive-datatable">
      ...
      <Column field="pto_emision" header="Pto. Emision"></Column>
      <Column field="secuencial" header="Secuencial"></Column>
...
export default {
  ...
  methods: {
    ...
    async ver(item) {
      ...
      this.nuevoRegistro.pto_emision = item.pto_emision;
    ...
    async editar(item) {
      ...
      this.nuevoRegistro.id_caja = item.id_caja;
      this.nuevoRegistro.nombre = item.nombre;
      this.nuevoRegistro.pto_emision = item.pto_emision;
      this.nuevoRegistro.estado = item.estado;
      const response = await admiSucursalService.getById(this.$api, item.sucursal_id);
    ...
    async actualizar() {
      try {
        ...
        const resCab = await this.$api.put(`${this.endpoint}/${this.nuevoRegistro.id_caja}`,
          this.nuevoRegistro
        );
        const respuesta = resCab.data;
        if (respuesta.success === true) {
    ...
    async eliminar(item) {
      try {
        const response = await this.$api.delete(`${this.endpoint}/${item.id_caja}`);
        const resp = response.data;
        if (resp.success === true) {
          this.listar();
        }
    ...
    limpiar() {
      ...
      this.view = false;
      this.update = false;
      this.nuevoRegistro = { nombre: "", pto_emision: "", estado: "", sucursalId: "" };
    },
    async restaurar(item) {
      try {
        item.estado = "Activo";
        const response = await this.$api.put(`${this.endpoint}/${item.id_caja}`, item);
        const resp = response.data;
        if (resp.success === true) {
          this.$swal.fire({
            icon: "success",
