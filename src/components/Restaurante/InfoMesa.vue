<template>
  <div class="responsive-dialog">
    <span class="text-title">Información de Mesas</span>
    <div class="grid">
      ...
      <div class="col-12 md:col-6 lg:col-3 xl:col-3">
        <AutoComplete id="estados_mesas"  placeholder="Filtra por estados:" v-model="estado_mesa"
          :suggestions="estadosArray" @complete="searchEstados"
          dropdown
          @keypress.enter="listar(estado_mesa, sucursal)" />
      </div>
      <div class="col-12 md:col-6 lg:col-3 xl:col-3">
        <AutoComplete id="sucursal"  placeholder="Filtra por sucursal:" v-model="sucursal"
          ...
          @keypress.enter="listar(estado_mesa, sucursal)" field="label" />
      </div>

    </div>
    <Dialog :header="dialogHeader" v-model:visible="mostrarDialogo" @hide="limpiar" class="responsive-dialog1">
      <div class="dialog-content">
        <InputText :disabled="view" id="numero_mesa" v-model="nuevoRegistro.numero_mesa" placeholder="# mesa:"
          class="input-spacing" />
      </div>
      <div class="dialog-content">
        <InputText :disabled="view" id="capacidad" v-model="nuevoRegistro.capacidad"
          placeholder="Capacidad:" class="input-spacing" />
      </div>
      <div class="dialog-content">
        <AutoComplete id="estados_mesas" :disabled="view" class="input-spacing" placeholder="Estado de la mesa:"
          v-model="nuevoRegistro.estado" :suggestions="estadosArray" @complete="searchEstados" dropdown
           />
      </div>
    ...
    <infoMesasEstadosDialog :registro="nuevoRegistro" :headerTitle="'Cambio de estado de mesas'" @registro-agregado="listar(estado_mesa, sucursal)"
        ref="infoMesasEstadosDialog"></infoMesasEstadosDialog>
      ...
      <Column field="options" header="Options">
        <template #body="slotProps">
          <div class="option-buttons">
            ...
            <Button icon="pi pi-trash" class="boton-rojo" @click="eliminar(slotProps.data)" v-tooltip="'Eliminar'"></Button>
            <Button icon="pi pi-history" class="boton-naranja" @click="cambiarEstadoMesa(slotProps.data)" v-tooltip="'Cambiar estado'"></Button>
          </div>
        </template>
      </Column>
      <Column field="numero_mesa" header="Numero Mesa"></Column>
...
export default {
  ...
  methods: {
    ...
    async ver(item) {
      ...
      this.nuevoRegistro.numero_mesa = item.numero_mesa;
    ...
    async editar(item) {
      ...
      this.nuevoRegistro.id_mesa = item.id_mesa;
      this.nuevoRegistro.numero_mesa = item.numero_mesa;
      this.nuevoRegistro.capacidad = item.capacidad;
      this.nuevoRegistro.estado = item.estado;
      const response = await admiSucursalService.getById(this.$api, item.sucursal_id);
    ...
    async actualizar() {
      try {
        ...
        const resCab = await this.$api.put(`${this.endpoint}/${this.nuevoRegistro.id_mesa}`,
          this.nuevoRegistro
        );
        const respuesta = resCab.data;
        if (respuesta.success === true) {
    ...
    async eliminar(item) {
      try {
        const response = await this.$api.delete(`${this.endpoint}/${item.id_mesa}`);
        const resp = response.data;
        if (resp.success === true) {
          this.listar();
        }
    ...
    limpiar() {
      ...
      this.view = false;
      this.update = false;
      this.nuevoRegistro = { numero_mesa: "", capacidad: "", estado: "",sucursalId:""};
    },
  },
};
</script>
