<template>
  <div class="responsive-dialog">
    <span class="text-title">Información de Platos</span>
    <div class="grid">
    ...
    <Dialog :header="dialogHeader" v-model:visible="mostrarDialogo" @hide="limpiar" class="responsive-dialog1">
      ...
      <div class="dialog-content">
        <span class="text-subtitle">Tiempo de preparación (min.):</span>

        <InputNumber :disabled="view" v-model="nuevoRegistro.tiempo" showButtons buttonLayout="vertical"
          style="width: 3rem" :min="0" :max="99">
    ...
    <DataTable :value="datos" class="responsive-datatable">
      ...
      <Column field="nombre" header="Plato"></Column>
...
export default {
  ...
  methods: {
    ...
    async ver(item) {
      ...
      this.nuevoRegistro.tiempo = item.tiempo;
    ...
    async editar(item) {
      ...
      this.nuevoRegistro.id_plato = item.id_plato;
      this.nuevoRegistro.nombre = item.nombre;
      this.nuevoRegistro.descripcion = item.descripcion;
      this.nuevoRegistro.precio = item.precio;
      this.nuevoRegistro.tiempo = item.tiempo;
    ...
    async actualizar() {
      try {
        ...
        this.nuevoRegistro.usuario_modificacion = this.$store.state.empleado.usuario;
        const resCab = await this.$api.put(`${this.endpoint}/${this.nuevoRegistro.id_plato}`,
          this.nuevoRegistro
        );
        const respuesta = resCab.data;
        if (respuesta.success === true) {
    ...
    async eliminar(item) {
      try {
        const response = await this.$api.delete(`${this.endpoint}/${item.id_plato}`);
        const resp = response.data;
        if (resp.success === true) {
          this.listar();
        }
    ...
    limpiar() {
      ...
      this.view = false;
      this.update = false;
      this.nuevoRegistro = { nombre: "", descripcion: "", estado: "", precio: 0.0, tiempo: 0 };
    },
    async restaurar(item) {
      try {
        item.estado = "Activo";
        const response = await this.$api.put(`${this.endpoint}/${item.id_plato}`, item);
        const resp = response.data;
        if (resp.success === true) {
          this.$swal.fire({
            icon: "success",
