<template>
  <div class="responsive-dialog">
    <span class="text-title">Información de Clientes</span>
    <div class="grid">
      ...
      <div  class="col-12 md:col-6 lg:col-3">
       ...
       <div  class="col-12 md:col-6 lg:col-3 xl:col-3">
        <AutoComplete id="estado_sistema" class="w-6" placeholder="Filtra por estados:" v-model="estado_sistema"
        :suggestions="estados_sistemas" @complete="searchBHerramienta"
         @keypress.enter="listar(estado_sistema)" dropdown />
       </div>
    </div>
    ...
    <DataTable :value="datos" class="responsive-datatable" :paginator="true" :rows="rows" :totalRecords="totalRecords" :lazy="true" :first="first" @page="onPage">
      ...
      <Column field="razon_social_comprador" header="Razon social"></Column>
      <Column field="tipoIdentificacionComprador" header="Tipo identificacion cliente"></Column>
...
import AdmiPersonaDialog from "../Administracion/options/AdmiPersonaDialog.vue";
...
export default {
  ...
  methods: {
   ...
   async ver(item) {
      ...
      this.nuevoRegistro.id_cliente = item.id_cliente;
      this.nuevoRegistro.id_persona = item.persona_id;
      this.nuevoRegistro.razon_social_comprador = item.razon_social_comprador;
      this.$refs.AdmiPersonaDialog.openDialogViewCliente();
    },
    async editar(item) {
      ...
      this.nuevoRegistro.id_persona = item.persona_id;
      this.nuevoRegistro.razon_social_comprador = item.razon_social_comprador;
      this.nuevoRegistro.tipoIdentificacionComprador = item.tipoIdentificacionComprador;
      this.$refs.AdmiPersonaDialog.openDialogUpdateCliente();
    },
    async eliminar(item) {
      try {
        const response = await this.$api.delete(`${this.endpoint}/${item.id_cliente}`);
        const resp = response.data;
        if (resp.success === true) {
          this.$swal.fire({
            icon:"success",
    ...
    async restaurar(item){
      try {
        item.estado="Activo";
        const response = await this.$api.put(`${this.endpoint}/${item.id_cliente}`,item);
        const resp = response.data;
        if (resp.success === true) {
          this.$swal.fire({
            icon:"success",
