  ...
  <TabView :activeIndex.sync="activeIndex" @tab-change="limpiarTabs">
    <TabPanel>
      ...
      <div class="grid">
        ...
        <div class="col-6">
          <AutoComplete id="mesas" :disabled="view" class="w-12" placeholder="Mesas:" v-model="nuevoRegistro.mesaId"
...
export default {
  ...
  methods: {
    ...
    async listarPedido(filtro = "", busqueda = {}) {
      try {
        this.limpiar();
        this.datosPedidos = [];
        let params = {
    ...
    async getEstadosPedidos() {
      try {
        ...
        if (respuesta.success === true) {
          this.estados_pedidosd = respuesta.data;
        }
      } catch (e) {
        const data = e.response.data;
        this.$swal.fire({
